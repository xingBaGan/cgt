...
module.exports = {
    run: function (payload, options) {
        ...
        if ('repository' in arg_input_map) {
        ...
        inquirer.prompt(questions).then(answers => {
            const projectName = answers['project-name'] || payload['project-name'];
            let repository = answers.repository || payload.repository;
